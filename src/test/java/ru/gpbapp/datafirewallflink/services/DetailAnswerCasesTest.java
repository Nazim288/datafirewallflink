...
class DetailAnswerCasesTest {
    ...
    static Stream<Case> cases() {
        return Stream.of(
                new Case("input/case1_normal.jsonl",         "answer/case1_detail_expected.json"),
                new Case("input/case2_null_mapping.jsonl",   "answer/case2_detail_expected.json"),
                new Case("input/case3_null_value.jsonl",     "answer/case3_detail_expected.json"),
                new Case("input/case4_none_mapping.jsonl",   "answer/case4_detail_expected.json"),
                new Case("input/case5_missing_fields.jsonl", "answer/case5_detail_expected.json")
        );
    }

    @ParameterizedTest
    @MethodSource("cases")
    void should_build_detail_answer(Case c) throws Exception {
        ...
        Map<String, Rule> rules = Map.of(
                "Rule1069", nameContainsCyrAndLat("ОСНОВНЫЕ СВЕДЕНИЯ.Имя"),
        ...
        DetailAnswerService detailService = new DetailAnswerService(MAPPER);
        String actual = detailService.build(original, vr);

        ObjectNode actualNode = (ObjectNode) MAPPER.readTree(actual);
        stripDetailDynamicFields(actualNode);
    ...
    private static void stripDetailDynamicFields(ObjectNode n) {
    ...
    private static String readResource(String classpathPath) throws Exception {
        try (InputStream is = DetailAnswerCasesTest.class.getClassLoader().getResourceAsStream(classpathPath)) {
